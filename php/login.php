 ...
 class Login
 {
     ...
     **/
     ...
     public static function getLoginByUserName(&$mysqli, $newUserName)
     {
         ...
         //sanitize and reset
         $cleanUser = filter_var($newUserName, FILTER_SANITIZE_STRING);
         if($cleanUser === false) {
             throw(new mysqli_sql_exception("Invalid username $newUserName"));
         }
     ...
     public static function getLoginByUserId(&$mysqli, $newUserId)
     {
         ...
         //sanitize and reset
         $cleanUserId = filter_var($newUserId, FILTER_SANITIZE_NUMBER_INT);
         if($cleanUserId === false) {
             throw(new mysqli_sql_exception("Invalid user id $newUserId"));
         }
     ...
     public static function getLoginByAuthenticationToken(&$mysqli, $newAuthenticationToken)
     {
         ...
         // second verify this is a string of 32 hexadecimal characters
         $filterOptions = array("options" => array("regexp" => "/^[\da-f]{32}$/"));
         $cleanAuthenticationToken = filter_var($newAuthenticationToken, FILTER_VALIDATE_REGEXP, $filterOptions);
         if ($cleanAuthenticationToken === false) {
             throw(new mysqli_sql_exception("$newAuthenticationToken is not hexadecimal"));
         }
         ...
         // bind the member variables to the place holders in the template
         $wasClean = $statement->bind_param("s", $cleanAuthenticationToken);
